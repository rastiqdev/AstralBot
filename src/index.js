...
if (client.logger.isDebugEnabled())
  client.logger.debug("Le mode de débuggage est activé");
...
if (!process.env.YOUTUBE_API_KEY) {
  client.logger.error(
    "La variable d'environnement 'YOUTUBE_API_KEY' n'existe pas."
  );
  process.exit(1);
}
...
client.player.on("error", (queue, error) => {
  client.logger.error(`Error emitted from the queue ${error.message}`);
});

client.player.on("connectionError", (queue, error) => {
  client.logger.error(`Error emitted from the connection ${error.message}`);
});

client.player.on("trackStart", (queue, track) => {
  queue.metadata.send(
    `Musique ${track.title} jouée dans **${queue.connection.channel.name}** 🎧`
  );
});
...
client.player.on("botDisconnect", queue => {
  queue.metadata.send(
    "J'été déconnecté manuellement du salon, je clear la queue... ❌"
  );
});
