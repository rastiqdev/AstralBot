...
module.exports = sendWelcomeImage = async function(client, member) {

    try {
        const canvas = new Jimp(500, 150);
        const avatar = await Jimp.read(member.user.displayAvatarURL({format: 'png'}));
  
        const Quantify_55_white = await Jimp.loadFont(path.join(__dirname, '../../res/fonts/Quantify_55_white.fnt'));
        const Quantify_25_white = await Jimp.loadFont(path.join(__dirname, '../../res/fonts/Quantify_25_white.fnt'));
        const OpenSans_22_white = await Jimp.loadFont(path.join(__dirname, '../../res/fonts/OpenSans_22_white.fnt'));
        ...
        return console.log('WelcomeImage => ',error);
      }
