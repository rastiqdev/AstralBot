...
module.exports = async function (client) {
  ...
  ).channels.fetch(client.config.subCounterChannelId);
  const viewsChannel = await (
    await client.guilds.fetch(client.config.mainGuildId)
  ).channels.fetch(client.config.viewsCounterChannelId);
  ...
  await subChannel.setName(
    `Abonnés : ${statistics.subscriberCount} ${
      statistics.subscriberCount.includes("69") ? "(nice)" : ""
    }`
  );
  await viewsChannel.setName(
    `Total de vues : ${statistics.viewCount} ${
      statistics.videoCount.includes("69") ? "(nice)" : ""
    }`
  );
