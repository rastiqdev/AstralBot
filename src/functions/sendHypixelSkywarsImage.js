...
module.exports = async function (interaction, player, username) {
  try {
    ...
    const avatar = await Jimp.read(
      `https://visage.surgeplay.com/bust/512/${uuid}`
    );

    const Quantify_50_white = await Jimp.loadFont(
      path.join(__dirname, "../../res/fonts/Quantify_50_white.fnt")
    );
    const Quantify_20_white = await Jimp.loadFont(
      path.join(__dirname, "../../res/fonts/Quantify_20_white.fnt")
    );
    ...
    if (player.stats.SkyWars.skywars_experience) {
      canvas.print(
        ...
        20,
        `Niveau ${Math.trunc(
          await xpToLevel(player.stats.SkyWars.skywars_experience)
        )}`
      );
    }
    if (player.achievements.skywars_wins_solo) {
      canvas.print(
        ...
        `Total de ${
          ...
          player.achievements.skywars_wins_team
        } wins`
      );
    }
    if (player.stats.SkyWars.losses_solo) {
      canvas.print(
        ...
        105,
        `Total de ${
          player.stats.SkyWars.losses_solo + player.stats.SkyWars.losses_team
        } parties perdues`
      );
    }
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        `Voici les stats Hypixel de ${player.displayname} en Bedwars`
      )
