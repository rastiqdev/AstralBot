...
module.exports = handleSuggestionVotes = async function(interaction) {
    const user = interaction.member.id;
    let author = await interaction.client.votesdb.get(interaction.message.id, "author")
    ...
    let votes = await interaction.client.votesdb.get(interaction.message.id, "votes");
