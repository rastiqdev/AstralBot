  ...
  async execute(client, interaction) {
    if (interaction.isCommand()) {
      ...
      if (command.voiceChannel) {
        if (!interaction.member.voice.channel)
          return interaction.reply({
            content: `${interaction.user}, vous n'êtes pas dans un salon vocal. ❌`,
            ephemeral: true,
          });
        ...
        if (
          ...
          return interaction.reply({
            content: `${interaction.user}, vous n'êtes pas dans le même salon vocal que moi. ❌`,
            ephemeral: true,
          });
      }
      ...
      try {
        ...
        return interaction.reply({
          content:
            "Une erreur s'est produite pendant l'exécution de cette commande !",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId === "commencer") {
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(
            "Vous n'êtes pas un robot ? Alors prouvez-le ! Pour cela, écrivez les lettres que " +
        ...
        collector.on("collect", async message => {
          ...
          if (message.content.toUpperCase() !== captcha.text) {
            ...
            if (invalid > 2 && toggle === true) {
              if (interaction.member.kickable) {
                ...
                interaction.member.send(
                  "Vous avez été kick pour trop d'essais de captcha invalides"
                );
                await interaction.member.kick(
                  "Trop d'essais de captcha invalides"
                );
                return;
              }
              collector.stop();
            ...
            await interaction.followUp({
              content: `:x: | **Code invalide, veuillez réessayer. Il vous reste ${
                3 - invalid
              } essais**`,
              ephemeral: true,
            });
            ...
            try {
              ...
              collector.stop();
              const rearow = new MessageActionRow().addComponents(
              ...
              const rearow2 = new MessageActionRow().addComponents(
                new MessageButton()
              ...
              const reaembed = new MessageEmbed()
                .setTitle(
                  "(2/2) · A présent, choisissez vos rôles de notification !"
                )
                .setColor("#0099ff")
                .setDescription(
                  "Maintenant que je suis sûr que vous n'êtes pas un robot, veuillez " +
                    "choisir ci-dessous les notifications que vous voulez recevoir (Vous pourrez toujours " +
                    "les changer plus tard)."
                );
              await interaction.followUp({
                embeds: [reaembed],
                components: [rearow, rearow2],
                ephemeral: true,
              });
            } catch (e) {
              collector.stop();
        ...
        collector.on("end", async (collected, reason) => {
          ...
          if (reason === "time" && toggle === true) {
            if (interaction.member.kickable) {
              ...
              await interaction.member.kick(
                "N'a pas répondu au captcha à temps"
              );
            }
          }
        });
        ...
        if (
          interaction.member.id === interaction.message.member.id ||
          ...
          await interaction.reply({
            content: "Vous n'êtes pas autorisé(e) à supprimer ce message",
            ephemeral: true,
          });
        }
        ...
        await interaction.reply({
          content:
            "Voulez-vous vraiment supprimer le ticket ? Cette action est irréversible",
          components: [
              ...
              embeds: [
                {
                  ...
                  description:
                    `Vous avez supprimé le ticket \`${channelName}\`, voici la transcription ` +
        ...
        if (!queue || !queue.playing)
          return interaction.reply({
            content: "Aucune musique est jouée. ❌",
            ephemeral: true,
            components: [],
          });
        ...
        interaction.member
          .send(
            `Vous avez sauvegardé la musique ${queue.current.title} | ${queue.current.author} dans le serveur ${interaction.member.guild.name} ✅`
          )
          .then(() => {
            return interaction.reply({
              content: "Je vous ai envoyé le nom de la musique en DM ✅",
              ephemeral: true,
              components: [],
            });
          })
          .catch(() => {
            return interaction.reply({
              content: "Je n'ai pas réussi à vous envoyer un DM ❌",
              ephemeral: true,
              components: [],
            });
          });
      ...
      if (customId === "select" || customId === "selectsetup") {
        ...
        for (const id of removed) {
          await member.roles.remove(
            interaction.guild.roles.cache.find(role => role.name === id.value)
              .id
          );
        }
        ...
        for (const id of values) {
          await member.roles.add(
            interaction.guild.roles.cache.find(role => role.name === id).id
          );
        }
