...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option
        .setName("utilisateur")
        .setDescription("L'utilisateur à qui enlever les warns")
        .setRequired(true)
    ),
  async execute(client, interaction) {
    ...
    if (user.bot) {
      return interaction.reply({
        content: "Les bots n'ont pas de warns.",
        ephemeral: true,
      });
    }
    ...
    if (!(await client.warnsdb.has(`${user.id}`, "warns"))) {
      return interaction.reply({
        content: `${user.tag} n'a pas de warns.`,
        ephemeral: true,
      });
    }
    ...
    await interaction.reply({
      content: `Les warns de ${user.user.tag} ont été supprimés !`,
      ephemeral: true,
    });
  },
};
