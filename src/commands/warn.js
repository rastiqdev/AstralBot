...
module.exports = {
	...
	async execute(client, interaction) {
        ...
        if(!await client.warnsdb.has(`${user.id}`, "warns")) {
          ...
          await user.send(`Vous avez été warn dans **${interaction.guild.name}** par ${author.user.username + "#" + author.user.discriminator} pour ${reason}`)
          ...
          await user.send(`Vous avez été warn dans **${interaction.guild.name}** par ${author.user.username + "#" + author.user.discriminator} pour ${reason}`)
          await interaction.reply({ content: `Vous avez warn **${user.user.username + "#" + user.user.discriminator}** pour \`${reason}\``, ephemeral: true })
          if (await client.warnsdb.get(`${interaction.guild.id}_${user.id}`, "warns") === 3) {
            ...
            await interaction.followUp({ content: `Cet utilisateur a maintenant 3 warns ! Voulez vous le bannir ?`, ephemeral: true, components: [row] });
            ...
            collector.on('collect', async i => {
              if (i.customId === 'oui') {
                ...
                await interaction.followUp({ content: `En train de bannir cet utilisateur...`, ephemeral: true })
